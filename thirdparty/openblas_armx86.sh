#!/bin/bash
#将ndk目录加入环境变量
export NDK_DIR=/home/liushunpeng/thirdparty/android-ndk-r16b
#将toolchain目录加入环境变量
export MY_TOOLCHAIN=${NDK_DIR}/mchain_x86_api23_gnustl
#生成独立工具链
...
cd OpenBLAS
make clean
prefix=`pwd`/install_armx86_r16b
rm -rf $prefix
#编译ARMV7环境静态库
make TARGET=x86 BINARY=32 ONLY_CBLAS=1 AR=$AR CC="clang ${CLANG_FLAGS}" HOSTCC=gcc ARM_SOFTFP_ABI=1 NO_SHARED=1 -j4
#安装静态库
